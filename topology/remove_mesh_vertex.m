...
%
%% Syntax
%   [face_new,vertex_new,father] = remove_mesh_vertex(face,vertex,index)
%
%% Description
%  face  : double array, nf x 3, connectivity of mesh
%  vertex: double array, nv x 3, vertex of mesh, there may have unreferenced
%          vertex
%  index : double array, n x 1, index of vertex to be removed
% 
...
function [face_new,vertex_new,father] = remove_mesh_vertex(face,vertex,index)
ind = false(size(vertex,1),1);
ind(index) = true;
ind_face = sum(ind(face),2)>0;
[face_new,vertex_new,father] = clean_mesh(face(~ind_face,:),vertex);
